...
import { Plus, Pencil, UserCircle2, Trash2 } from "lucide-react";
...
export default function UsersPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (form.sector_id && form.sector_id !== "none" && data.user_id) {
        await supabase.from("profiles").update({ sector_id: form.sector_id }).eq("user_id", data.user_id);
      }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (editUser.role_id) {
        await supabase.from("user_roles").update({ role: editRole }).eq("id", editUser.role_id);
      } else {
        await supabase.from("user_roles").insert({ user_id: editUser.user_id, role: editRole });
      }
              ...
              <Card key={u.id} className="border-0 shadow-sm">
                <CardContent className="flex items-center justify-between p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground leading-tight">{u.name}</h3>
                      <p className="text-xs text-muted-foreground">{sectorName(u.sector_id)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {u.role ? (
                      <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${roleColors[u.role]}`}>
                        {roleLabels[u.role]}
                      </span>
                    ) : (
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1">
              ...
              <Input id="c-name" placeholder="João da Silva" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
            </div>
            <div className="space-y-1">
              <Label htmlFor="c-email">E-mail (login)</Label>
              <Input id="c-email" type="email" placeholder="joao@maqz.com" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} />
            </div>
            <div className="space-y-1">
              <Label htmlFor="c-password">Senha inicial</Label>
              <Input id="c-password" type="password" placeholder="Mínimo 6 caracteres" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} />
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Select value={form.sector_id} onValueChange={(v) => setForm((f) => ({ ...f, sector_id: v }))}>
                <SelectTrigger id="c-sector"><SelectValue placeholder="Selecionar setor" /></SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={saving} className="bg-accent text-accent-foreground hover:bg-accent/90">
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1">
              ...
              <Select value={editSector} onValueChange={setEditSector}>
                <SelectTrigger id="e-sector"><SelectValue placeholder="Selecionar setor" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Sem setor</SelectItem>
                  {sectors.map((s) => (<SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>))}
                </SelectContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleEdit} disabled={saving} className="bg-primary text-primary-foreground hover:bg-primary/90">
      ...
      <AlertDialog open={!!deleteUser} onOpenChange={(open) => !open && setDeleteUser(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir <strong>{deleteUser?.name}</strong>? Esta ação não pode ser desfeita. Todas as atribuições de tarefas deste usuário serão removidas.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
